...
final class ConvergencePlotNodeModel extends OptimizationPlotNodeModel {
    ...
    protected BufferedDataTable[] execute(final BufferedDataTable[] input,
        ...
        for (DataRow row : input[0]) {
            ...
            final double lower = lowerIndex < 0 ? y
                : number(row.getCell(lowerIndex), row, m_lower.getStringValue());
            final double upper = upperIndex < 0 ? y
                : number(row.getCell(upperIndex), row, m_upper.getStringValue());
        ...
        setPlotData(new OptimizationPlotData("Convergence plot", m_nfe.getStringValue(),
            m_value.getStringValue(), false, m_logX.getBooleanValue(), m_logY.getBooleanValue(),
            List.copyOf(series)));
    ...
    @Override
    OptimizationPlotData emptyPlot() {
        return OptimizationPlotData.empty("Convergence plot", m_nfe.getStringValue(),
            m_value.getStringValue(), false);
    }
