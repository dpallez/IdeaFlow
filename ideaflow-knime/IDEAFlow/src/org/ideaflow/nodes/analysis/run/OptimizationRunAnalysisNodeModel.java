...
final class OptimizationRunAnalysisNodeModel extends NodeModel {
  ...
  private final SettingsModelString m_direction = new SettingsModelString(CFG_DIRECTION, "AUTO");
  ...
  private Map<Key, Trajectory> read(
      final BufferedDataTable input, final DataTableSpec spec, final ExecutionContext execution)
      throws Exception {
    final Map<Key, Trajectory> result =
        new TreeMap<>(
            (left, right) -> {
    ...
    for (DataRow row : input) {
      final String population = text(row, populationIndex);
      final long seed = longNumber(row.getCell(seedIndex), row, OptimizationSummary.SEED);
      final Key key = new Key(text(row, seriesIndex), text(row, runIndex), population, seed);
      final String problem = text(row, problemIndex);
      ...
      if (!value.isMissing()) {
        trajectory.values.put(nfe, KnimeTableSupport.number(value, row, m_value.getStringValue()));
      }
  ...
  private void writeResults(
      final Map<Key, Trajectory> all,
      final OptimizationDirection direction,
      final BufferedDataContainer convergence,
      final BufferedDataContainer ecdf,
      final BufferedDataContainer runs,
    ...
    for (Map.Entry<String, List<Trajectory>> seriesEntry : bySeries.entrySet()) {
      ...
      for (long checkpoint : checkpoints) {
        ...
        convergence.addRowToTable(
            new DefaultRow(
                "Convergence-" + convergenceRow++,
                new DataCell[] {
                  new StringCell(series),
                  new LongCell(checkpoint),
                  new IntCell(values.length),
                  new DoubleCell(quantile(values, 0.5)),
                  new DoubleCell(quantile(values, 0.25)),
                  new DoubleCell(quantile(values, 0.75)),
                  new DoubleCell(mean),
                  new DoubleCell(standardDeviation)
                }));
        execution.checkCanceled();
      }
      ...
      for (Trajectory trajectory : trajectories) {
        ...
        runs.addRowToTable(
            new DefaultRow(
                ...
                new DataCell[] {
                  new StringCell(series),
                  new StringCell(trajectory.problem),
                  new StringCell(trajectory.key.run),
                  new StringCell(trajectory.key.population),
  ...
  private double[] valuesAt(final List<Trajectory> trajectories, final long checkpoint) {
    ...
    int count = 0;
    for (Trajectory trajectory : trajectories) {
      final Map.Entry<Long, Double> entry =
          m_carryForward.getBooleanValue()
              ? trajectory.values.floorEntry(checkpoint)
              : trajectory.values.ceilingEntry(checkpoint);
  ...
  private static int addEcdf(
    ...
    output.addRowToTable(
        new DefaultRow(
            ...
            new DataCell[] {
              new StringCell(series),
              new DoubleCell(fitness),
              new IntCell(atOrBelow),
              new IntCell(total),
  ...
  private OptimizationDirection direction(final DataTableSpec spec)
    ...
    return ProblemMetadata.require(spec).objectives().get(0).direction();
  }

  private void validate(final DataTableSpec spec) throws InvalidSettingsException {
    requireText(spec, m_series.getStringValue());
    requireText(spec, m_run.getStringValue());
    requireText(spec, OptimizationSummary.POPULATION);
    requireText(spec, OptimizationSummary.PROBLEM);
    KnimeTableSupport.requireNumericColumns(
        spec, List.of(m_nfe.getStringValue(), m_value.getStringValue(), OptimizationSummary.SEED));
    ...
    if ("AUTO".equals(direction)) direction(spec);
  ...
  private static String text(final DataRow row, final int index) throws InvalidSettingsException {
    if (row.getCell(index).isMissing()) {
      throw new InvalidSettingsException(
          "A required analysis grouping value is missing at " + row.getKey());
    }
  ...
  private static long longNumber(final DataCell cell, final DataRow row, final String column)
    ...
    throw new InvalidSettingsException(
        "Missing or non-numeric value in '" + column + "' at " + row.getKey());
  }
  ...
  static DataTableSpec convergenceSpec() {
    return new DataTableSpec(
        KnimeTableSupport.stringColumn(OptimizationSummary.SERIES),
        KnimeTableSupport.longColumn(PopulationState.NFE),
        KnimeTableSupport.intColumn("Runs"),
        KnimeTableSupport.doubleColumn("Median"),
        KnimeTableSupport.doubleColumn("Q1"),
        KnimeTableSupport.doubleColumn("Q3"),
  ...
  static DataTableSpec ecdfSpec() {
    return new DataTableSpec(
        KnimeTableSupport.stringColumn(OptimizationSummary.SERIES),
        KnimeTableSupport.doubleColumn("Fitness"),
        KnimeTableSupport.intColumn("Runs at or below"),
        KnimeTableSupport.intColumn("Total runs"),
        KnimeTableSupport.doubleColumn("ECDF"));
  }
  ...
  static DataTableSpec runSpec() {
    return new DataTableSpec(
        ...
        KnimeTableSupport.longColumn("Final NFE"),
        KnimeTableSupport.doubleColumn("Final value"),
