...
final class EcdfPlotNodeModel extends OptimizationPlotNodeModel {
    ...
    // Retain the old settings key so existing ECDF Plot nodes migrate without load errors.
    static final String CFG_FITNESS = "nfe_column";
    ...
    protected BufferedDataTable[] execute(final BufferedDataTable[] input,
        ...
        final String fitnessColumn = fitnessColumn(spec);
    ...
    private void validate(final DataTableSpec spec) throws InvalidSettingsException {
        ...
        KnimeTableSupport.requireNumericColumns(spec,
            List.of(fitnessColumn(spec), m_ecdf.getStringValue()));
    }

    private String fitnessColumn(final DataTableSpec spec) {
        final String configured = m_fitness.getStringValue();
        if ("NFE".equals(configured) && spec.findColumnIndex(configured) < 0
                && spec.findColumnIndex("Fitness") >= 0) {
            return "Fitness";
        }
        return configured;
    }
