...
public final class OptimizationPlotPanel extends JPanel {
  ...
  public static BufferedImage renderImage(
      final OptimizationPlotData data, final int width, final int height) {
  ...
  // Compute bounds in transformed coordinates so linear and logarithmic rendering share the same
  // path.
  ...
  private static void drawGridAndAxes(
      final Graphics2D g,
      final OptimizationPlotData data,
      final Bounds bounds,
      final int left,
      final int right,
      final int top,
    ...
    g.drawString(
        data.xLabel(),
        (left + right - g.getFontMetrics().stringWidth(data.xLabel())) / 2,
        bottom + 48);
    g.rotate(-Math.PI / 2);
    g.drawString(
        data.yLabel(), -(top + bottom + g.getFontMetrics().stringWidth(data.yLabel())) / 2, 25);
    g.rotate(Math.PI / 2);
  }
  ...
  private static void drawSeries(
      final Graphics2D g,
      final OptimizationPlotData data,
      final OptimizationPlotData.Series series,
      final Color color,
      final Bounds bounds,
      final int left,
      final int right,
    ...
    for (OptimizationPlotData.Point point : series.points()) {
      ...
      points.add(
          new ScreenPoint(
              toX(transformedX, bounds, left, right),
              toY(transformedY, bounds, top, bottom),
              point));
    }
  ...
  // The uncertainty band is drawn before the line so the median remains legible.
  ...
  private static String label(final double transformed, final boolean logarithmic) {
    ...
    return new DecimalFormat(absolute >= 100.0 ? "0" : absolute >= 1.0 ? "0.##" : "0.###")
        .format(value);
  }

  private static int toX(final double value, final Bounds bounds, final int left, final int right) {
    return (int)
        Math.round(left + (value - bounds.minX) / (bounds.maxX - bounds.minX) * (right - left));
  }

  private static int toY(final double value, final Bounds bounds, final int top, final int bottom) {
    return (int)
        Math.round(bottom - (value - bounds.minY) / (bounds.maxY - bounds.minY) * (bottom - top));
  }
