...
final class EcdfPlotNodeModel extends OptimizationPlotNodeModel {
  ...
  private final SettingsModelString m_fitness = new SettingsModelString(CFG_FITNESS, "Fitness");
  ...
  EcdfPlotNodeModel() {
    super(OptimizationPlotData.empty("Final-fitness ECDF", "Fitness", "Fraction of runs", true));
  }
      ...
      if (Double.isFinite(x) && Double.isFinite(y)) {
        if (y < 0.0 || y > 1.0) {
          throw new InvalidSettingsException(
              "ECDF values must be between 0 and 1; found " + y + " at " + row.getKey());
        }
        grouped
  ...
  private void validate(final DataTableSpec spec) throws InvalidSettingsException {
    ...
    KnimeTableSupport.requireNumericColumns(
        spec, List.of(m_fitness.getStringValue(), m_ecdf.getStringValue()));
  }
  ...
  protected OptimizationPlotData emptyPlot() {
    return OptimizationPlotData.empty(
        "Final-fitness ECDF", m_fitness.getStringValue(), "Fraction of runs", true);
  }
