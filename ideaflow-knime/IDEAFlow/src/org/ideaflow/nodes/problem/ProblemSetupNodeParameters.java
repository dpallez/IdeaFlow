...
final class ProblemSetupNodeParameters implements NodeParameters {

  @Section(
      title = "Run settings",
      description =
          "Configure the single optimization run created by this node. Use KNIME loops for repeated runs.")
  interface RunSection {}

  @Section(
      title = "Variables",
      description = "Define the values explored by the optimization algorithm.")
  ...
  @Section(
      title = "Constraints",
      description =
          "Optional limits applied to numeric results produced by the evaluation workflow.")
  ...
  enum ConstraintRelationChoice {
    @Label(
        ...
        description = "The result must not exceed the threshold.")
    LE,
    @Label(
        value = "Greater than or equal to (≥)",
        description = "The result must reach the threshold.")
    GE,
    @Label(
        value = "Equal to (=)",
        description = "The result must be within the configured tolerance.")
    EQ
  }
  ...
  @Widget(
      title = "Seed",
      description =
          "The population and stochastic operators use this seed to make the run reproducible.")
  @Layout(RunSection.class)
  long m_seed = 42L;
  ...
  @Widget(
      ...
      description =
          "Exact-evaluation budget recorded for this run. Loop stopping nodes can use the same budget.")
  @NumberInputWidget(minValidation = IsPositiveIntegerValidation.class)
  @Layout(RunSection.class)
  int m_maxEvaluations = 10_000;

  @Widget(
      title = "Direct variables and groups",
      description =
          "Use a count of one for a single variable, or a larger count to generate a numbered group.")
  @ArrayWidget(
      addButtonText = "Add variable or group",
      elementTitle = "Variable group",
  ...
  @Widget(
      title = "Binary-encoded numerical groups",
      description =
          "Advanced: represent each logical number with several binary population columns.")
  @ArrayWidget(
      addButtonText = "Add binary-encoded group",
      elementTitle = "Encoded group",
      showSortButtons = true)
  @Layout(VariablesSection.class)
  ...
  @Widget(
      title = "Objectives",
      description = "Add one card for every objective column created by the evaluator.")
  ...
  @Widget(
      ...
      description =
          "Declare business limits such as temperature ≤ 80; IdeaFlow calculates violations automatically.")
  @ArrayWidget(
      addButtonText = "Add constraint",
      elementTitle = "Constraint",
      showSortButtons = true)
  ...
  static final class DirectVariableGroup implements NodeParameters {
    ...
    @Widget(
        title = "Name",
        description = "Column name for one variable, or prefix for a numbered group.")
    @TextInputWidget(placeholder = "x")
    String m_prefix = "x";

    @Widget(
        title = "Number of variables",
        description = "One creates the exact name above; larger values add indices.")
    @NumberInputWidget(
        minValidation = IsPositiveIntegerValidation.class,
        maxValidation = AtMost100000.class)
    int m_count = 2;

    @Widget(
        title = "Type",
        description = "How each variable is represented in the population table.")
    @ValueSwitchWidget
    ...
    @Widget(
        ...
        description = "Inclusive lower bound for every variable in this group.")
    @Effect(predicate = UsesBounds.class, type = EffectType.SHOW)
    double m_lower = -5.0;

    @Widget(
        title = "Largest value",
        description = "Inclusive upper bound for every variable in this group.")
    @Effect(predicate = UsesBounds.class, type = EffectType.SHOW)
    double m_upper = 5.0;
  }
  ...
  static final class EncodedVariableGroup implements NodeParameters {
    @Widget(
        title = "Decoded name",
        description = "Name for one decoded value, or prefix for a numbered vector.")
    @TextInputWidget(placeholder = "x")
    String m_prefix = "x";

    @Widget(
        title = "Number of numerical values",
        description = "Dimension of the decoded numerical vector.")
    @NumberInputWidget(
        minValidation = IsPositiveIntegerValidation.class,
        maxValidation = AtMost100000.class)
    int m_count = 10;
    ...
    @Widget(
        title = "Smallest decoded value",
        description = "Lower bound after decoding the binary genes.")
    double m_lower = -5.0;

    @Widget(
        title = "Largest decoded value",
        description = "Upper bound after decoding the binary genes.")
    double m_upper = 5.0;

    @Widget(
        title = "Bits per value",
        description = "Number of binary genes used to represent each numerical value.")
    @NumberInputWidget(
        minValidation = IsPositiveIntegerValidation.class,
        maxValidation = AtMost52.class)
    int m_bits = 10;
  }
  ...
  static final class Objective implements NodeParameters {
    ...
    @Widget(
        title = "Result column",
        description = "Numeric column created by the evaluation workflow.")
    @TextInputWidget(placeholder = "fitness")
    String m_name = "fitness";

    @Widget(
        title = "Optimization direction",
        description = "Choose whether smaller or larger values are better.")
    @ValueSwitchWidget
    Direction m_direction = Direction.MINIMIZE;

    @Widget(
        title = "Use a hypervolume reference value",
    ...
    @Widget(
        ...
        description = "A value worse than the region of interest for this objective.")
  ...
  static final class Constraint implements NodeParameters {
    @Widget(
        title = "Result column",
        description =
            "Numeric column produced by the evaluation workflow, for example temperature.")
    ...
    @Widget(
        title = "Relation",
        description = "How the evaluated value is compared with the threshold.")
    @ValueSwitchWidget
    ...
    @Widget(
        title = "Threshold",
        description = "Numeric limit used by the relation, for example 80.")
    double m_threshold;

    @Widget(
        title = "Tolerance",
        description =
            "Allowed absolute difference around the threshold for an equality constraint.")
    @NumberInputWidget(minValidation = IsNonNegativeValidation.class)
    @Effect(predicate = IsEquality.class, type = EffectType.SHOW)
    double m_tolerance;
  }

  // The preview catches naming collisions early, before the node is configured.
  static final class PreviewProvider implements StateProvider<Optional<Message>> {
      ...
      if (encoded != null) {
        for (EncodedVariableGroup group : encoded) {
          if (group != null && group.m_count > 0 && group.m_bits > 0) {
            ...
            examples
                .append(" → ")
                .append(group.m_count * (long) group.m_bits)
                .append(" genes (")
    ...
    private static void appendExample(
        final StringBuilder output, final String prefix, final int count, final String suffix) {
  ...
  @Override
  // Validate the complete logical namespace because generated gene and result columns share one
  // table.
  public void validate() throws InvalidSettingsException {
    ...
    if (m_directVariables != null) {
      for (DirectVariableGroup group : m_directVariables) {
        ...
        if (group.m_type == null)
          throw new InvalidSettingsException("Select a kind of value for every group.");
        if (group.m_type != VariableType.BINARY)
          validateBounds(group.m_prefix, group.m_lower, group.m_upper);
        if (group.m_type == VariableType.INTEGER
            && (group.m_lower != Math.rint(group.m_lower)
                || group.m_upper != Math.rint(group.m_upper))) {
          throw new InvalidSettingsException(
              "Whole-number bounds must be integers for " + group.m_prefix + '.');
        }
    ...
    if (m_encodedVariables != null) {
      for (EncodedVariableGroup group : m_encodedVariables) {
        ...
        if (group.m_count < 1 || group.m_bits < 1 || group.m_bits > 52) {
          throw new InvalidSettingsException(
              "Encoded counts and bits must be positive; bits may not exceed 52.");
        }
        validateBounds(group.m_prefix, group.m_lower, group.m_upper);
        for (int i = 0; i < group.m_count; i++) {
    ...
    if (variableCount == 0)
      throw new InvalidSettingsException("Add at least one variable or encoded group.");
    ...
    for (Objective objective : m_objectives) {
      ...
      if (objective.m_direction == null)
        throw new InvalidSettingsException("Every objective needs a direction.");
    ...
    if (references != 0 && references != m_objectives.length) {
      throw new InvalidSettingsException(
          "Enable a reference value for every objective, or for none of them.");
    }
    if (m_constraints != null) {
      for (Constraint constraint : m_constraints) {
        ...
        if (!Double.isFinite(constraint.m_tolerance) || constraint.m_tolerance < 0.0) {
          throw new InvalidSettingsException(
              "Equality tolerances must be finite and non-negative.");
        }
  ...
  private static void validateBounds(final String name, final double lower, final double upper)
    ...
    if (!Double.isFinite(lower) || !Double.isFinite(upper) || lower >= upper) {
      throw new InvalidSettingsException(
          "The smallest value must be below the largest value for " + name + '.');
    }
  }

  private static void requirePrefix(final String prefix) throws InvalidSettingsException {
    if (prefix == null || prefix.isBlank())
      throw new InvalidSettingsException("Every variable group needs a name.");
  }

  private static void requireUnique(final Set<String> names, final String rawName)
      throws InvalidSettingsException {
    final String name = rawName.trim();
    if (name.isEmpty())
      throw new InvalidSettingsException("Generated column names may not be empty.");
