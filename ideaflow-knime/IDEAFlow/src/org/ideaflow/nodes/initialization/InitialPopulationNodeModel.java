...
/** Creates reproducible candidate rows from the connected problem definition. */
public final class InitialPopulationNodeModel extends NodeModel {
  ...
  private final SettingsModelString m_populationId =
      new SettingsModelString(CFG_POPULATION_ID, "population-0");
  ...
  protected BufferedDataTable[] execute(
    ...
    if (variables.isEmpty())
      throw new InvalidSettingsException("Problem Definition contains no variable rows.");
    final ProblemMetadata.Schema metadata =
        ProblemMetadata.read(input[0].getDataTableSpec()).orElse(null);
    ...
    for (int individual = 0; individual < m_populationSize.getIntValue(); individual++) {
      final RandomGenerator random =
          DeterministicRandom.forScope(
              run.seed(), run.id(), m_populationId.getStringValue(), "initialization", individual);
  ...
  private static DataCell directValue(final Variable variable, final RandomGenerator random)
    ...
    return switch (variable.type()) {
      case "REAL" ->
          new DoubleCell(random.nextDouble(variable.lower(), Math.nextUp(variable.upper())));
      case "INTEGER" -> {
        ...
        if (lower < Integer.MIN_VALUE || upperExclusive - 1 > Integer.MAX_VALUE) {
          throw new InvalidSettingsException(
              "Whole-number bounds exceed KNIME Integer range: " + variable.name());
        }
        yield new IntCell((int) random.nextLong(lower, upperExclusive));
      }
      case "BINARY" -> new IntCell(random.nextBoolean() ? 1 : 0);
      default ->
          throw new InvalidSettingsException("Unsupported variable type: " + variable.type());
    };
  }

  // Runtime rows are checked against metadata so malformed Problem Setup tables fail clearly.
  private static List<Variable> readVariables(final BufferedDataTable problem)
      throws InvalidSettingsException {
    ...
    for (DataRow row : problem) {
      ...
      if (expectedProblem == null) expectedProblem = rowProblem;
        ...
        throw new InvalidSettingsException(
            "Population Initialization accepts one problem definition at a time.");
      }
      ...
      final String variableRepresentation =
          row.getCell(representation).toString().toUpperCase(Locale.ROOT);
      final String variableEncoding = row.getCell(encoding).toString().toUpperCase(Locale.ROOT);
      final int variableBits =
          row.getCell(bits).isMissing() ? 1 : ((IntValue) row.getCell(bits)).getIntValue();
      if (low > high)
        throw new InvalidSettingsException(
            "Lower bound exceeds upper bound for " + row.getCell(name));
  ...
  private static Run readRun(final BufferedDataTable setup) throws InvalidSettingsException {
    ...
    for (DataRow row : setup) {
      ...
      if (current.id().isBlank())
        throw new InvalidSettingsException("Problem Setup contains an empty run ID.");
    ...
    if (result == null)
      throw new InvalidSettingsException("Problem Setup contains no definition rows.");
    return result;
  }
  ...
  // Keep visible identifiers first, decision variables next, and the internal state cell last.
  private static DataTableSpec outputSpec(
      final List<Variable> variables, final ProblemMetadata.Schema metadata)
      throws InvalidSettingsException {
    ...
    return metadata == null
        ? result
        : ProblemMetadata.attach(result, PopulationState.COLUMN, metadata);
  }
  ...
  private static void addColumn(
      ...
      throws InvalidSettingsException {
    if (!names.add(name))
      throw new InvalidSettingsException("Duplicate generated population column: " + name);
    columns.add(new DataColumnSpecCreator(name, type).createSpec());
  }

  private static void requireProblemSetup(final DataTableSpec spec)
      throws InvalidSettingsException {
    for (String required :
        List.of(
            "experiment_id",
      ...
      if (spec.findColumnIndex(required) < 0) {
        throw new InvalidSettingsException(
            "Input must come from Problem Setup; missing " + required);
      }
    }
  }

  private void validateSettingsValues() throws InvalidSettingsException {
    if (m_populationId.getStringValue().isBlank())
      throw new InvalidSettingsException("Population ID is required.");
  }
