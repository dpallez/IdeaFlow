...
/**
 * Classic-workbench fallback; the modern dialog hides these fields instead of merely disabling
 * them.
 */
final class SelectionNodeDialog extends DefaultNodeSettingsPane {
  SelectionNodeDialog() {
    final SettingsModelString strategy =
        new SettingsModelString(SelectionNodeModel.CFG_MODE, "TOURNAMENT");
    final SettingsModelIntegerBounded tournament =
        new SettingsModelIntegerBounded(
            SelectionNodeModel.CFG_TOURNAMENT_SIZE, 2, 1, Integer.MAX_VALUE);
    final SettingsModelIntegerBounded count =
        new SettingsModelIntegerBounded(
            SelectionNodeModel.CFG_PARENT_COUNT, 100, 1, Integer.MAX_VALUE);
    final SettingsModelBoolean replacement =
        new SettingsModelBoolean(SelectionNodeModel.CFG_WITH_REPLACEMENT, true);
    final SettingsModelDoubleBounded pbest =
        new SettingsModelDoubleBounded(SelectionNodeModel.CFG_PBEST, 0.2, Double.MIN_NORMAL, 1.0);
    ...
    addDialogComponent(
        new DialogComponentStringSelection(
            strategy, "Selection strategy", "TOURNAMENT", "RANDOM", "DE_DONORS"));
