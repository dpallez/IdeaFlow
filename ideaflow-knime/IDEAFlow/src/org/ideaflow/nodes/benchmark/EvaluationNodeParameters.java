...
final class EvaluationNodeParameters implements NodeParameters {
  ...
  static final class FormulaCardsPersistor implements NodeParametersPersistor<FormulaCard[]> {
    @Override
    public FormulaCard[] load(final NodeSettingsRO settings) throws InvalidSettingsException {
      return EvaluationNodeModel.decodeFormulaSettings(
              settings.getString(EvaluationNodeModel.CFG_FORMULAS))
          .stream()
          .map(
              definition -> {
    ...
    @Override
    public void save(final FormulaCard[] value, final NodeSettingsWO settings) {
      ...
      settings.addString(
          EvaluationNodeModel.CFG_FORMULAS, EvaluationNodeModel.encodeFormulaSettings(definitions));
    }

    @Override
    public String[][] getConfigPaths() {
      return new String[][] {{EvaluationNodeModel.CFG_FORMULAS}};
    }
  }

  static final class FormulaHelp
      implements org.knime.node.parameters.updates.StateProvider<Optional<Message>> {
    @Override
              ...
              .flatMap(
                  spec -> {
                    try {
                      return Optional.of(
                          String.join(
                              ", ", ProblemMetadata.require(spec).evaluatorVariableNames()));
                    } catch (InvalidSettingsException exception) {
  ...
  @Widget(
      title = "Results and formulas",
      description =
          "Add one card for every objective and constraint result declared in Problem Setup.")
  @ArrayWidget(
      addButtonText = "Add result formula",
      elementTitle = "Result formula",
      ...
      for (FormulaCard formula : m_formulas) {
        ...
        if (!results.add(formula.m_result)) {
          throw new InvalidSettingsException(
              "A result can only be calculated once: " + formula.m_result);
        }
        try {
