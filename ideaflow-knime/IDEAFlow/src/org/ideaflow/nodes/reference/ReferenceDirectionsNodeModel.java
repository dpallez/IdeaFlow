...
/** Generates Das-Dennis directions and stores them in each candidate state. */
final class ReferenceDirectionsNodeModel extends NodeModel {
  static final String CFG_DIVISIONS = "divisions";
  private final SettingsModelIntegerBounded divisions =
      new SettingsModelIntegerBounded(CFG_DIVISIONS, 12, 1, 1000);
  ...
  protected BufferedDataTable[] execute(
    ...
    final List<double[]> directions =
        ReferenceDirections.dasDennis(objectiveCount, divisions.getIntValue());
  ...
  private DataTableSpec spec(final int objectiveCount) {
    ...
    columns.add(
        new org.knime.core.data.DataColumnSpecCreator("direction_id", StringCell.TYPE)
            .createSpec());
    for (int index = 0; index < objectiveCount; index++) {
      columns.add(
          new org.knime.core.data.DataColumnSpecCreator("weight_" + index, DoubleCell.TYPE)
              .createSpec());
    }
  ...
  private static int objectiveCount(final DataTableSpec spec) throws InvalidSettingsException {
    ...
    if (count < 2)
      throw new InvalidSettingsException("Reference Directions requires at least two objectives.");
    return count;
  }
    ...
    for (int i = 1; i <= objectiveCount - 1; i++) {
      ...
      if (count > 1_000_000)
        throw new InvalidSettingsException(
            "The settings create more than one million directions; reduce objectives or divisions.");
    }
  }

  @Override
  protected void saveSettingsTo(final NodeSettingsWO settings) {
    divisions.saveSettingsTo(settings);
  }

  @Override
  protected void validateSettings(final NodeSettingsRO settings) throws InvalidSettingsException {
    divisions.validateSettings(settings);
  }

  @Override
  protected void loadValidatedSettingsFrom(final NodeSettingsRO settings)
      throws InvalidSettingsException {
    divisions.loadSettingsFrom(settings);
  }
