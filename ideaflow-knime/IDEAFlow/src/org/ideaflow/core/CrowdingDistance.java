...
public final class CrowdingDistance {
    ...
    public static double[] compute(final List<Candidate> population, final List<List<Integer>> fronts,
        ...
        for (List<Integer> front : fronts) {
            ...
            for (int objective = 0; objective < objectives.size(); objective++) {
                ...
                ordered.sort(Comparator.comparingDouble(index -> objectives.get(objectiveIndex).direction()
                ...
                final double min = objectives.get(objective).direction().normalize(
                        population.get(ordered.get(0)).objectives()[objective]);
                final double max = objectives.get(objective).direction().normalize(
                        population.get(ordered.get(ordered.size() - 1)).objectives()[objective]);
                if (max == min) continue;
                for (int i = 1; i < ordered.size() - 1; i++) {
                    if (Double.isInfinite(result[ordered.get(i)])) continue;
                    final double previous = objectives.get(objective).direction().normalize(
                            population.get(ordered.get(i - 1)).objectives()[objective]);
                    final double next = objectives.get(objective).direction().normalize(
                            population.get(ordered.get(i + 1)).objectives()[objective]);
