...
public final class QualityIndicators {
  ...
  public static double generationalDistance(
      final List<Candidate> approximation,
      final List<Candidate> reference,
      ...
      for (Candidate candidate : approximation) {
        ...
        for (int dimension = 0; dimension < objectives.size(); dimension++) {
          final double candidateValue =
              objectives.get(dimension).direction().normalize(candidate.objectives()[dimension]);
          final double referenceValue =
              objectives.get(dimension).direction().normalize(ref.objectives()[dimension]);
  ...
  public static double spacing(
      final List<Candidate> approximation, final List<ObjectiveDefinition> objectives) {
    ...
    for (int first = 0; first < approximation.size(); first++) {
      ...
      for (int second = 0; second < approximation.size(); second++)
        if (first != second) {
          double distance = 0.0;
          for (int dimension = 0; dimension < objectives.size(); dimension++) {
            distance +=
                Math.abs(
                    objectives
                            .get(dimension)
                            .direction()
                            .normalize(approximation.get(first).objectives()[dimension])
                        - objectives
                            .get(dimension)
                            .direction()
                            .normalize(approximation.get(second).objectives()[dimension]));
          }
    ...
    double variance = 0.0;
    for (double value : nearest) variance += (value - mean) * (value - mean);
    return Math.sqrt(variance / (nearest.length - 1)) / mean;
  }

  public static double additiveEpsilon(
      final List<Candidate> approximation,
      final List<Candidate> reference,
      final List<ObjectiveDefinition> objectives) {
    requireNonEmpty(approximation, reference);
  ...
  private static double averageNearestDistance(
      final List<Candidate> source,
      final List<Candidate> target,
      final List<ObjectiveDefinition> objectives) {
      ...
      for (Candidate candidate : target) {
        ...
        for (int d = 0; d < objectives.size(); d++) {
          final double difference =
              objectives.get(d).direction().normalize(point.objectives()[d])
  ...
  private static void requireNonEmpty(final List<?> first, final List<?> second) {
    if (first.isEmpty() || second.isEmpty())
      throw new IllegalArgumentException("Indicator sets cannot be empty.");
  }
}
