...
public final class ParetoDominance {
    ...
    public static int compare(final Candidate left, final Candidate right,
        ...
        if (a.length != objectives.size() || b.length != objectives.size()) {
            throw new IllegalArgumentException("Candidate objective count does not match problem definition.");
        }
        ...
        for (int i = 0; i < objectives.size(); i++) {
            requireFinite(a[i]);
            requireFinite(b[i]);
            final int comparison = Double.compare(
