...
public final class Hypervolume {
    ...
    public static double compute(final List<Candidate> approximation,
            final List<ObjectiveDefinition> objectives, final double[] referencePoint) {
        ...
        for (int d = 0; d < reference.length; d++) {
            ...
            if (!Double.isFinite(reference[d])) throw new IllegalArgumentException("Reference point must be finite.");
        }
        ...
        for (Candidate candidate : approximation) {
            ...
            if (raw.length != objectives.size()) throw new IllegalArgumentException("Objective count mismatch.");
    ...
    public static double[] contributions(final List<Candidate> approximation,
            final List<ObjectiveDefinition> objectives, final double[] referencePoint) {
    ...
    private static double unionVolume(final List<double[]> points, final double[] reference, final int dimensions) {
        ...
        final List<Double> boundaries = points.stream().map(point -> point[axis]).distinct().sorted().toList();
        double volume = 0.0;
        for (int i = 0; i < boundaries.size(); i++) {
            ...
            volume += (upper - lower) * unionVolume(removeDominated(active), Arrays.copyOf(reference, axis), axis);
        }
        return volume;
    }
    ...
    private static List<double[]> removeDominated(final List<double[]> points) {
        ...
        outer: for (int i = 0; i < points.size(); i++) {
