...
/**
 * Small deterministic expression language for objective and constraint formulas.
 * It intentionally has no scripting, reflection, file, or network access.
 */
public final class FormulaExpression {
    ...
    private record NumberExpression(double value) implements Expression {
        @Override public double evaluate(final Map<String, Double> variables) { return value; }
        @Override public void variables(final Set<String> output) { }
    }

    private record VariableExpression(String name) implements Expression {
        @Override public double evaluate(final Map<String, Double> variables) {
            final Double value = variables.get(name);
            if (value == null) throw new IllegalArgumentException("No value was provided for variable: " + name);
            return value;
        }
        @Override public void variables(final Set<String> output) { output.add(name); }
    }
    ...
    private record UnaryExpression(char operator, Expression value) implements Expression {
        ...
        @Override public void variables(final Set<String> output) { value.variables(output); }
    }
    ...
    private record FunctionExpression(String name, List<Expression> arguments) implements Expression {
        @Override public double evaluate(final Map<String, Double> variables) {
            final double[] values = arguments.stream().mapToDouble(argument -> argument.evaluate(variables)).toArray();
            return switch (name) {
    ...
    public static FormulaExpression compile(final String source) {
        if (source == null || source.isBlank()) throw new IllegalArgumentException("Formula must not be empty.");
    ...
    public Set<String> variables() { return m_variables; }
    ...
    private static final class Parser {
        ...
        Parser(final String source) { m_source = source; }
        ...
        Expression primary() {
            ...
            if (Character.isJavaIdentifierStart(current())) {
                ...
                final int expected = switch (function) {
                    case "abs", "sqrt", "exp", "log", "ln", "log10", "sin", "cos", "tan",
                            "floor", "ceil", "round" -> 1;
                    case "min", "max", "pow" -> 2;
        ...
        boolean finished() { return m_position >= m_source.length(); }
        char current() { return m_source.charAt(m_position); }
