...
public final class FastNonDominatedSort {
    ...
    public static List<List<Integer>> sort(final List<Candidate> population,
        ...
        for (int p = 0; p < size; p++) {
            ...
            for (int q = 0; q < size; q++) {
                if (p == q) continue;
                final int relation = ParetoDominance.compare(population.get(p), population.get(q), objectives);
    ...
    public static int[] ranks(final List<Candidate> population, final List<ObjectiveDefinition> objectives) {
