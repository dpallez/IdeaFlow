...
public final class KnimeTableSupport {
    ...
    public static int[] requireNumericColumns(final DataTableSpec spec, final List<String> names)
            throws InvalidSettingsException {
        if (names.isEmpty()) throw new InvalidSettingsException("At least one column must be selected.");
    ...
    public static double number(final DataCell cell, final DataRow row, final String column)
        ...
        else throw new InvalidSettingsException("Non-numeric value in '" + column + "' at " + row.getKey());
        if (!Double.isFinite(value)) throw new InvalidSettingsException("Non-finite value in '" + column + "' at " + row.getKey());
        return value;
    }

    public static List<ObjectiveDefinition> objectives(final List<String> columns,
            final List<OptimizationDirection> directions, final List<Double> references) {
        final List<ObjectiveDefinition> result = new ArrayList<>();
        for (int i = 0; i < columns.size(); i++) {
            result.add(new ObjectiveDefinition(columns.get(i), directions.get(i), null,
                    references == null || references.isEmpty() ? null : references.get(i)));
        }
        return List.copyOf(result);
    }

    public static Candidate candidate(final DataRow row, final int[] variableIndices,
            final int[] objectiveIndices, final int violationIndex, final List<String> variableNames,
            final List<String> objectiveNames) throws InvalidSettingsException {
        final double[] variables = new double[variableIndices.length];
        final double[] objectives = new double[objectiveIndices.length];
        for (int i = 0; i < variables.length; i++) variables[i] = number(row.getCell(variableIndices[i]), row, variableNames.get(i));
        for (int i = 0; i < objectives.length; i++) objectives[i] = number(row.getCell(objectiveIndices[i]), row, objectiveNames.get(i));
        final double violation = violationIndex < 0 ? 0.0
                : Math.max(0.0, number(row.getCell(violationIndex), row, ReservedColumns.CONSTRAINT_VIOLATION));
        return new Candidate(row.getKey().getString(), variables, objectives, violation);
    }

    public static DataTableSpec appendOrReplace(final DataTableSpec input, final DataColumnSpec... columns) {
        ...
        for (DataColumnSpec column : columns) {
            ...
            if (existing >= 0) result.set(existing, column); else result.add(column);
        }
        ...
        final int stateIndex = java.util.stream.IntStream.range(0, result.size())
    ...
    public static DataCell[] copyToSpec(final DataRow row, final DataTableSpec input, final DataTableSpec output) {
     ...
     *
     * <p>This is nested in an established Eclipse source unit so PDE incremental
     * builds discover it reliably when an existing workspace is refreshed.</p>
     */
    public static final class ProblemMetadata {
        public static final String PROPERTY_KEY = "ideaflow.problem.metadata.v1";
        private static final int FORMAT_VERSION = 2;

        public record Variable(String name, String type, double lower, double upper,
                String representation, String encoding, int bits) {
            public Variable {
        ...
        public record Objective(String column, OptimizationDirection direction, Double referencePoint) {
            public Objective {
                ...
                if (direction == null) throw new IllegalArgumentException("Objective direction is required.");
        ...
        public record Constraint(String column, ConstraintRelation relation, double threshold, double tolerance) {
            public Constraint {
                column = required(column, "Constraint column");
                if (relation == null) throw new IllegalArgumentException("Constraint relation is required.");
        ...
        public record Schema(String problemId, long maxEvaluations, List<Variable> variables,
            ...
            public Schema {
                ...
                if (variables.isEmpty()) throw new IllegalArgumentException("At least one variable is required.");
        ...
        public static DataTableSpec attach(final DataTableSpec input, final String markerColumn,
                final Schema schema) {
            final int marker = input.findColumnIndex(markerColumn);
            if (marker < 0) throw new IllegalArgumentException("Metadata marker column is missing: " + markerColumn);
            final DataColumnSpec[] columns = new DataColumnSpec[input.getNumColumns()];
            for (int index = 0; index < columns.length; index++) columns[index] = input.getColumnSpec(index);
            final DataColumnSpec original = columns[marker];
            final DataColumnSpecCreator creator = new DataColumnSpecCreator(original);
            creator.setProperties(original.getProperties().cloneAndOverwrite(Map.of(PROPERTY_KEY, encode(schema))));
        ...
        public static Schema require(final DataTableSpec spec) throws InvalidSettingsException {
            return read(spec).orElseThrow(() -> new InvalidSettingsException(
            ...
            try {
                ...
                try (DataInputStream input = new DataInputStream(new ByteArrayInputStream(bytes))) {
                    ...
                    for (int index = 0; index < variableCount; index++) {
                        variables.add(new Variable(input.readUTF(), input.readUTF(), input.readDouble(),
                            input.readDouble(), input.readUTF(), input.readUTF(), input.readInt()));
                    }
                    ...
                    for (int index = 0; index < constraintCount; index++) {
                        constraints.add(new Constraint(input.readUTF(),
                            ConstraintRelation.valueOf(input.readUTF()), input.readDouble(), input.readDouble()));
                    }
