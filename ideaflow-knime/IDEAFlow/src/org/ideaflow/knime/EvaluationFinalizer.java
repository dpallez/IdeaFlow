...
public final class EvaluationFinalizer {
    ...
    public record ProblemDetails(String problemId, List<Objective> objectives,
            List<ConstraintDefinition> constraints, ProblemMetadata.Schema metadata) {
    ...
    public static ProblemDetails fromMetadata(final DataTableSpec problemSpec) throws InvalidSettingsException {
        final ProblemMetadata.Schema metadata = ProblemMetadata.require(problemSpec);
        return new ProblemDetails(metadata.problemId(), metadata.objectives().stream()
            .map(objective -> new Objective(objective.column(), objective.direction())).toList(),
            metadata.constraints().stream().map(constraint -> new ConstraintDefinition(constraint.column(),
                constraint.relation(), constraint.threshold(), constraint.tolerance())).toList(), metadata);
    }
    ...
    public static ProblemDetails readProblemDefinition(final BufferedDataTable table)
        ...
        for (DataRow row : table) {
            ...
            if ("objective".equalsIgnoreCase(kind)) {
                ...
                objectives.add(new Objective(column,
                    OptimizationDirection.parse(requiredText(row.getCell(directionIndex), "direction"))));
                ...
                constraints.add(new ConstraintDefinition(column,
        ...
        if (problemId == null) throw new InvalidSettingsException("Problem definition must not be empty.");
        if (objectives.isEmpty()) {
            throw new InvalidSettingsException("Problem definition must contain an objective.");
        }
        return new ProblemDetails(problemId, objectives, constraints,
            ProblemMetadata.read(spec).orElse(null));
    }

    public static DataTableSpec evaluatedPopulationSpec(final DataTableSpec input,
        ...
        if (!problem.constraints().isEmpty()) {
            ...
            additions.add(new org.knime.core.data.DataColumnSpecCreator(
                PopulationState.FEASIBLE, BooleanCell.TYPE).createSpec());
        }
    ...
    public static DataTableSpec eventSpec(final ProblemDetails problem) {
        ...
        if (!problem.constraints().isEmpty()) {
            ...
            columns.add(new org.knime.core.data.DataColumnSpecCreator(
                PopulationState.FEASIBLE, BooleanCell.TYPE).createSpec());
        }
        ...
        return problem.metadata() == null ? result
            : ProblemMetadata.attach(result, "Run", problem.metadata());
    }

    public static BufferedDataTable[] finalizePopulation(final BufferedDataTable population,
            final ProblemDetails problem, final ExecutionContext execution, final String operator) throws Exception {
        ...
        final List<String> objectiveNames = problem.objectives().stream().map(Objective::column).toList();
        final List<String> constraintNames =
            problem.constraints().stream().map(ConstraintDefinition::column).toList();
        final int[] objectiveIndices = KnimeTableSupport.requireNumericColumns(inputSpec, objectiveNames);
        final int[] constraintIndices = constraintNames.isEmpty() ? new int[0]
        ...
        unevaluatedCount.forEach((run, count) ->
            finalNfe.put(run, previousNfe.getOrDefault(run, 0L) + count));
        ...
        for (DataRow row : population) {
            ...
            for (int index = 0; index < constraintIndices.length; index++) {
                ...
                violation += constraint.violation(KnimeTableSupport.number(
            ...
            for (int index = 0; index < objectives.length; index++) {
                objectives[index] = KnimeTableSupport.number(
                    row.getCell(objectiveIndices[index]), row, objectiveNames.get(index));
            }

            final long evaluation = alreadyEvaluated
            ...
            state = state.with(IdeaFlowState.EVALUATED, true)
                .with(IdeaFlowState.EVALUATION, evaluation);
    ...
    public static void validateEvaluationColumns(final DataTableSpec spec, final ProblemDetails problem)
            throws InvalidSettingsException {
    ...
    public static void requirePopulationStructure(final DataTableSpec spec) throws InvalidSettingsException {
        PopulationState.require(spec);
    }

    public static void requireProblemDefinitionStructure(final DataTableSpec spec)
            throws InvalidSettingsException {
        for (String required : List.of("problem_id", "kind", "name", "direction", "relation",
            "threshold", "tolerance")) {
