...
public final class OptimizationSummary {
    ...
    public record Entry(String run, String population, long nfe, int populationSize, int feasibleSize,
            double best, double mean, double worst, double standardDeviation, int nondominatedSize,
            double hypervolume, String series, long seed, String problem) {
        public DataCell[] cells() {
            return new DataCell[]{new StringCell(run), new StringCell(population), new LongCell(nfe),
                new IntCell(populationSize), new IntCell(feasibleSize), finite(best), finite(mean),
                finite(worst), finite(standardDeviation), new IntCell(nondominatedSize), finite(hypervolume),
                new StringCell(series), new LongCell(seed), new StringCell(problem)};
        }
    }
    ...
    public static Entry read(final DataRow row, final DataTableSpec spec) throws InvalidSettingsException {
        validate(spec);
        return new Entry(text(row, spec, RUN), text(row, spec, POPULATION),
            ...
            number(row, spec, BEST), number(row, spec, MEAN), number(row, spec, WORST),
    ...
    private static DataTableSpec rawSpec() {
        return new DataTableSpec(KnimeTableSupport.stringColumn(RUN),
            KnimeTableSupport.stringColumn(POPULATION),
            KnimeTableSupport.longColumn(PopulationState.NFE),
            KnimeTableSupport.intColumn(POPULATION_SIZE), KnimeTableSupport.intColumn(FEASIBLE_SIZE),
            KnimeTableSupport.doubleColumn(BEST), KnimeTableSupport.doubleColumn(MEAN),
            KnimeTableSupport.doubleColumn(WORST), KnimeTableSupport.doubleColumn(STANDARD_DEVIATION),
            KnimeTableSupport.intColumn(NONDOMINATED_SIZE), KnimeTableSupport.doubleColumn(HYPERVOLUME),
            KnimeTableSupport.stringColumn(SERIES), KnimeTableSupport.longColumn(SEED),
    ...
    private static String text(final DataRow row, final DataTableSpec spec, final String name)
        ...
        if (cell.isMissing()) throw new InvalidSettingsException("Progress summary value is missing: " + name);
