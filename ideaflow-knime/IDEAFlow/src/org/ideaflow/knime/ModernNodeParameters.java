...
public final class ModernNodeParameters {
    ...
    public static final class DirectionChoices implements StringChoicesProvider {
        @Override public List<StringChoice> computeState(final NodeParametersInput context) {
            return List.of(new StringChoice("MINIMIZE", "Minimize - smaller is better"),
    ...
    public static final class TargetRuleChoices implements StringChoicesProvider {
        @Override public List<StringChoice> computeState(final NodeParametersInput context) {
            return List.of(new StringChoice("ALL", "All targets - one individual must reach every target"),
            ...
            return context.getInTableSpec(0).map(spec -> {
                try {
                    return KnimeTableSupport.ProblemMetadata.require(spec).objectives().stream()
                        .map(objective -> new StringChoice(objective.column(),
                            objective.column() + " (" + objective.direction().name().toLowerCase() + ")"))
                        .toList();
    ...
    public static final class MigrationTopologyChoices implements StringChoicesProvider {
        @Override public List<StringChoice> computeState(final NodeParametersInput context) {
            return List.of(new StringChoice("RING", "Ring - A to B, B to C, C to A"),
    ...
    public static final class MigrationReplacementChoices implements StringChoicesProvider {
        @Override public List<StringChoice> computeState(final NodeParametersInput context) {
            return List.of(new StringChoice("REPLACE_WORST", "Replace the weakest candidates"),
    ...
    public static final class RepairChoices implements StringChoicesProvider {
        @Override public List<StringChoice> computeState(final NodeParametersInput context) {
            return List.of(new StringChoice("REFLECT", "Reflect at the boundary"),
    ...
    public static final class NumericColumnChoices implements ColumnChoicesProvider {
        @Override public List<DataColumnSpec> columnChoices(final NodeParametersInput context) {
            return context.getInTableSpec(0).map(spec -> {
                ...
                for (DataColumnSpec column : spec) {
                    ...
                    if (!ReservedColumns.isReserved(column.getName()) && (DoubleValue.class.isAssignableFrom(value)
                            || IntValue.class.isAssignableFrom(value) || LongValue.class.isAssignableFrom(value))) {
                        result.add(column);
                    }
                }
    ...
    private abstract static class CsvArrayPersistor implements NodeParametersPersistor<String[]> {
        ...
        CsvArrayPersistor(final String key) { m_key = key; }
        @Override public String[] load(final NodeSettingsRO settings) throws InvalidSettingsException {
            ...
            return value == null || value.isBlank() ? new String[0]
                : Arrays.stream(value.split(",")).map(String::trim).filter(item -> !item.isEmpty()).toArray(String[]::new);
        }
        ...
        @Override public String[][] getConfigPaths() { return new String[][]{{m_key}}; }
    }
    public static final class AdditionalPropertiesPersistor extends CsvArrayPersistor {
        public AdditionalPropertiesPersistor() { super("additional_properties"); }
    }
    ...
    private abstract static class EnumNamePersistor<E extends Enum<E>>
        ...
        @Override public E load(final NodeSettingsRO settings) throws InvalidSettingsException {
            ...
            try {
                ...
                throw new InvalidSettingsException("Unsupported setting for " + m_key + ": " + value, exception);
            }
        }
        @Override public void save(final E value, final NodeSettingsWO settings) {
    ...
    public static final class TargetCondition implements NodeParameters {
        ...
        @Widget(title="Target value",
            description="Stop when this value is reached according to the objective's minimize/maximize direction.")
    ...
    public static final class TargetConditionsPersistor implements NodeParametersPersistor<TargetCondition[]> {
        ...
        @Override public TargetCondition[] load(final NodeSettingsRO settings) throws InvalidSettingsException {
            ...
            try {
                for (int index = 0; index < items.length; index++) {
                    ...
                    condition.objective = new String(java.util.Base64.getUrlDecoder()
                        .decode(items[index].substring(0, separator)), java.nio.charset.StandardCharsets.UTF_8);
        ...
        @Override public void save(final TargetCondition[] value, final NodeSettingsWO settings) {
            if (value == null || value.length == 0) {
                settings.addString(KEY, "");
                return;
            }
            settings.addString(KEY, java.util.Arrays.stream(value).map(condition ->
                java.util.Base64.getUrlEncoder().withoutPadding().encodeToString(
                    condition.objective.getBytes(java.nio.charset.StandardCharsets.UTF_8))
                    + ":" + Double.toString(condition.value)).collect(java.util.stream.Collectors.joining(",")));
        }
    ...
    public static final class ParentSelection implements NodeParameters {
        enum Strategy {
            ...
            @Label(value="Differential Evolution donors",
                description="Prepare target and donor vectors for DE mutation.") DE_DONORS
        }
        ...
        static final class StrategyPersistor extends EnumNamePersistor<Strategy> {
            StrategyPersistor() { super("selection_strategy", Strategy.class, Strategy.TOURNAMENT); }
        }
        ...
        static final class UsesOrdinarySelection implements EffectPredicateProvider {
            @Override public EffectPredicate init(final PredicateInitializer initializer) {
                return initializer.getEnum(StrategyRef.class)
                    .isOneOf(Strategy.TOURNAMENT, Strategy.RANDOM);
            }
        }
        ...
        @ValueSwitchWidget @ValueReference(StrategyRef.class) @Persistor(StrategyPersistor.class)
        ...
        @Persist(configKey="tournament_size") public int tournamentSize=2;
        @Widget(title="Parents per population", description="Number of selected parent rows per run and population.")
        @Effect(predicate=UsesOrdinarySelection.class, type=EffectType.SHOW)
        @Persist(configKey="parent_count") public int parentCount=100;
        @Widget(title="Allow duplicate selections", description="Sample parents with replacement.")
        @Effect(predicate=UsesOrdinarySelection.class, type=EffectType.SHOW)
        @Persist(configKey="with_replacement") public boolean withReplacement=true;
        @Widget(title="DE p-best rate", description="Top fraction eligible as p-best donors for current-to-pbest mutation.")
    ...
    public static final class Recombination implements NodeParameters {
        enum Strategy {
            ...
            @Label(value="One-point", description="Split and exchange sequences at one point.") ONE_POINT,
            @Label(value="Arithmetic", description="Blend continuous parent values.") ARITHMETIC,
            @Label(value="DE binomial", description="Binomial Differential Evolution crossover.") DE_BINOMIAL,
            @Label(value="DE exponential", description="Exponential Differential Evolution crossover.") DE_EXPONENTIAL
        }
        interface StrategyRef extends ParameterReference<Strategy> { }
        static final class StrategyPersistor extends EnumNamePersistor<Strategy> {
            StrategyPersistor() { super("strategy", Strategy.class, Strategy.SBX); }
        }
        static final class UsesOrdinaryCrossover implements EffectPredicateProvider {
            @Override public EffectPredicate init(final PredicateInitializer initializer) {
                return initializer.getEnum(StrategyRef.class).isOneOf(
                    Strategy.SBX, Strategy.UNIFORM, Strategy.ONE_POINT, Strategy.ARITHMETIC);
            }
        }
        ...
        @ValueReference(StrategyRef.class) @Persistor(StrategyPersistor.class)
        Strategy strategy=Strategy.SBX;
        @Widget(title="Crossover probability", description="Probability of recombining each parent pair.")
        @Effect(predicate=UsesOrdinaryCrossover.class, type=EffectType.SHOW)
        @Persist(configKey="probability") public double probability=0.9;
        @Widget(title="SBX distribution index", description="Larger values create children closer to their parents.")
        @Effect(predicate=UsesSbx.class, type=EffectType.SHOW)
        @Persist(configKey="distribution_index") public double distributionIndex=20.0;
    }

    public static final class Mutation implements NodeParameters {
        enum Strategy {
            @Label(value="Polynomial", description="Bounded mutation for continuous variables.") POLYNOMIAL,
            @Label(value="Gaussian", description="Add normally distributed noise to continuous variables.") GAUSSIAN,
            @Label(value="Bit flip", description="Invert binary values.") BIT_FLIP,
            @Label(value="Random reset", description="Draw a new integer or categorical value.") RANDOM_RESET,
            @Label(value="DE/rand/1", description="Random base plus one difference vector.") DE_RAND_1,
            @Label(value="DE/best/1", description="Best candidate as the base vector.") DE_BEST_1,
            @Label(value="DE/current-to-best/1", description="Move the target toward the best candidate.") DE_CURRENT_TO_BEST_1,
            @Label(value="DE/current-to-pbest/1",
        ...
        static final class StrategyPersistor extends EnumNamePersistor<Strategy> {
            StrategyPersistor() { super("strategy", Strategy.class, Strategy.POLYNOMIAL); }
        }
        static final class AdaptationPersistor extends EnumNamePersistor<Adaptation> {
            AdaptationPersistor() { super("adaptation_mode", Adaptation.class, Adaptation.FIXED); }
        }
        ...
        static final class UsesManualRate implements EffectPredicateProvider {
            @Override public EffectPredicate init(final PredicateInitializer initializer) {
                return ordinary(initializer).and(
                    initializer.getBoolean(AutoRateRef.class).isFalse());
            }
        }
        ...
        static final class UsesJde implements EffectPredicateProvider {
            @Override public EffectPredicate init(final PredicateInitializer initializer) {
                return differential(initializer).and(
                    initializer.getEnum(AdaptationRef.class).isOneOf(Adaptation.JDE));
            }
        }
        static final class UsesShade implements EffectPredicateProvider {
            @Override public EffectPredicate init(final PredicateInitializer initializer) {
                return differential(initializer).and(
                    initializer.getEnum(AdaptationRef.class).isOneOf(Adaptation.SHADE));
            }
        }
        @Widget(title="Strategy", description="Mutation strategy.")
        @ValueReference(StrategyRef.class) @Persistor(StrategyPersistor.class)
        Strategy strategy=Strategy.POLYNOMIAL;
        @Widget(title="Automatic mutation rate", description="Use one divided by the variable count.")
        @ValueReference(AutoRateRef.class)
        @Effect(predicate=UsesOrdinaryMutation.class, type=EffectType.SHOW)
        @Persist(configKey="auto_rate") public boolean autoRate=true;
        @Widget(title="Manual mutation probability", description="Used only when automatic probability is disabled.")
        @Effect(predicate=UsesManualRate.class, type=EffectType.SHOW)
        @Persist(configKey="mutation_rate") public double mutationRate=0.1;
        @Widget(title="Gaussian mutation strength", description="Standard deviation as a fraction of the variable range.")
        @Effect(predicate=UsesGaussian.class, type=EffectType.SHOW)
        @Persist(configKey="gaussian_scale") public double gaussianScale=0.1;
        @Widget(title="Polynomial distribution index", description="Larger values make smaller local changes.")
        @Effect(predicate=UsesPolynomial.class, type=EffectType.SHOW)
        @Persist(configKey="distribution_index") public double distributionIndex=20.0;
        @Widget(title="DE parameter control", description="Use fixed F and CR, jDE self-adaptation, or SHADE success-history adaptation.")
        @ValueSwitchWidget @ValueReference(AdaptationRef.class)
        @Effect(predicate=UsesDifferentialMutation.class, type=EffectType.SHOW)
        @Persistor(AdaptationPersistor.class) Adaptation adaptation=Adaptation.FIXED;
        @Widget(title="Initial or fixed F", description="Differential weight used directly in fixed mode or as the initial adaptive value.")
        @Effect(predicate=UsesDifferentialMutation.class, type=EffectType.SHOW)
        @Persistor(DifferentialWeightPersistor.class) public double f=0.5;
        @Widget(title="Initial or fixed CR", description="Crossover rate stored for DE Crossover; also initializes jDE or SHADE.")
        @Effect(predicate=UsesDifferentialMutation.class, type=EffectType.SHOW)
        @Persistor(InitialCrPersistor.class) public double cr=0.9;
        @Widget(title="jDE F adaptation probability", description="Probability that an individual samples a new F.")
        @Effect(predicate=UsesJde.class, type=EffectType.SHOW)
        @Persistor(TauFPersistor.class) public double tauF=0.1;
        @Widget(title="jDE CR adaptation probability", description="Probability that an individual samples a new CR.")
        @Effect(predicate=UsesJde.class, type=EffectType.SHOW)
        @Persistor(TauCrPersistor.class) public double tauCr=0.1;
        ...
        @Persistor(MemorySizePersistor.class) public int memorySize=6;
        @Widget(title="DE bounds repair", description="How out-of-range differential mutants are repaired.")
    ...
    public static final class PopulationUpdate implements NodeParameters {
        enum Mode {
            ...
            @Label(value="NSGA-II", description="Use Pareto rank and crowding.") NSGA_II,
            @Label(value="NSGA-III", description="Use Pareto rank and reference directions.") NSGA_III,
            @Label(value="GDE3", description="Use constraint-aware multiobjective DE competition.") GDE3
        }
        enum SizePolicy {
            @Label(value="Fixed", description="Keep the current population size.") FIXED,
            @Label(value="Linear NFE reduction",
                description="Reduce the population linearly as evaluations are consumed.") LINEAR_NFE
        }
        ...
        static final class ModePersistor extends EnumNamePersistor<Mode> {
            ModePersistor() { super("update_mode", Mode.class, Mode.NSGA_II); }
        }
        static final class SizePolicyPersistor extends EnumNamePersistor<SizePolicy> {
            SizePolicyPersistor() { super("population_size_policy", SizePolicy.class, SizePolicy.FIXED); }
        }
        ...
        @ValueReference(ModeRef.class) @Persistor(ModePersistor.class)
        Mode mode=Mode.NSGA_II;
        @Widget(title="NSGA-III reference divisions", description="Controls the resolution of NSGA-III reference directions.")
        @Effect(predicate=UsesNsga3.class, type=EffectType.SHOW)
        @Persist(configKey="reference_divisions") public int referenceDivisions=12;
        @Widget(title="Population size policy", description="Keep the original size, or reduce it linearly according to NFE for L-SHADE.")
        @ValueSwitchWidget @ValueReference(SizePolicyRef.class)
        @Persistor(SizePolicyPersistor.class) SizePolicy populationSizePolicy=SizePolicy.FIXED;
        @Widget(title="Final population size", description="Population size at the maximum NFE. Used only with linear reduction.")
    ...
    public static final class EvolutionLoopEnd implements NodeParameters {
        ...
        @Widget(title="Objective targets",
            description="Optional stopping targets. With no targets, the loop stops only at the NFE budget.")
        @ArrayWidget(addButtonText="Add objective target", elementTitle="Objective target", showSortButtons=true)
        ...
        @Widget(title="When several targets are configured",
            description="Choose whether any target is enough, or one individual must reach all targets.")
    ...
    public static final class OptimizationRunAnalysis implements NodeParameters {
        ...
        @Widget(title="Run column", description="Stable identifier for one independent optimization run.")
        @Persist(configKey="run_column") public String runColumn="Run";
        @Widget(title="Evaluation-budget column", description="Horizontal-axis evaluation count.")
        @Persist(configKey="nfe_column") public String nfeColumn="NFE";
        @Widget(title="Performance column",
            description="Scalar value summarized for convergence and compared with the ECDF target.")
        @Persist(configKey="performance_column") public String performanceColumn="Best";
        @Widget(title="Preferred direction",
            description="Infer from IdeaFlow metadata, or explicitly choose whether smaller or larger values are better.")
        @ChoicesProvider(AnalysisDirectionChoices.class)
        @Persist(configKey="performance_direction") public String direction="AUTO";
        @Widget(title="Per-run success target",
            description="Used only for Success and First hit NFE in the Runs output; the final-fitness ECDF does not use a target.")
        @Persist(configKey="target") public double target=0.1;
        @Widget(title="Carry observations forward",
            description="Use the most recent value when runs record different NFE checkpoints.")
        @Persist(configKey="carry_forward") public boolean carryForward=true;
    }
    ...
    public static final class ConvergencePlot implements NodeParameters {
        ...
        @Persist(configKey="series_column") public String seriesColumn="Series";
        @Widget(title="Horizontal-axis column", description="Evaluation count shown on the horizontal axis.")
        @Persist(configKey="nfe_column") public String nfeColumn="NFE";
        @Widget(title="Convergence value column",
            description="Central convergence statistic, normally the median across runs.")
        @Persist(configKey="value_column") public String valueColumn="Median";
        @Widget(title="Lower band column",
            description="Lower uncertainty boundary, normally Q1. Leave blank to hide the band.")
        @Persist(configKey="lower_column") public String lowerColumn="Q1";
        @Widget(title="Upper band column",
            description="Upper uncertainty boundary, normally Q3. Leave blank to hide the band.")
        @Persist(configKey="upper_column") public String upperColumn="Q3";
        @Widget(title="Logarithmic horizontal axis",
            description="Show positive evaluation counts on a base-10 logarithmic scale.")
        @Persist(configKey="log_x") public boolean logX=false;
        @Widget(title="Logarithmic vertical axis",
            description="Show positive convergence values on a base-10 logarithmic scale.")
        @Persist(configKey="log_y") public boolean logY=false;
    }
    ...
    public static final class AnalysisDirectionChoices implements StringChoicesProvider {
        @Override public List<StringChoice> computeState(final NodeParametersInput context) {
            return List.of(new StringChoice("AUTO", "Automatic - use IdeaFlow metadata"),
    ...
    public static final class PopulationMigration implements NodeParameters {
        ...
        @Widget(title="When migrants arrive",
            description="Replace weak candidates to preserve population sizes, or add migrants and allow sizes to grow.")
    ...
    public static final class IohProfilerExport implements NodeParameters {
        ...
        @Widget(title="Problem ID", description="IOH function or problem identifier.") @Persist(configKey="problem_id") public String problemId="1";
        @Widget(title="Algorithm", description="Algorithm name stored in metadata.") @Persist(configKey="algorithm") public String algorithm="IdeaFlow";
        @Widget(title="Algorithm information", description="Free-form algorithm description.") @Persist(configKey="algorithm_info") public String info="";
        @Widget(title="Scalar performance column", description="Select an objective for single-objective runs or hypervolume for multi-objective trajectories.")
        @ChoicesProvider(NumericColumnChoices.class) @Persist(configKey="raw_y_column") public String rawY="";
        @Widget(title="Instance", description="IOH problem instance number.") @Persist(configKey="instance") public int instance=1;
        @Widget(title="Write complete .cdat log", description="Also emit every event in a complete data file.") @Persist(configKey="write_complete") public boolean complete=true;
        @Widget(title="Additional sidecar columns", description="Optional columns retained in the lossless companion file.")
        @ChoicesProvider(AllColumnChoices.class) @Persistor(AdditionalPropertiesPersistor.class) public String[] properties=new String[0];
    }
}
